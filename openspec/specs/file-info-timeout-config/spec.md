# file-info-timeout-config

## Purpose

Operators can bound the WinFsp kernel `FileInfo` cache lifetime via `RamDrive:FileInfoTimeoutMs`, with a safe default that prevents indefinite stale-cache survival if a notification is missed. Acts as defence in depth on top of `cache-invalidation`.
